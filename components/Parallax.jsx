...
const ImageClient = dynamic(() => import('next/image'), {
        ...
        <div className="container mx-auto text-center px-4">
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
            {items.map((item, index) => {
              return (
                <div className='flex flex-col justify-center items-center text-white z-10 group' key={index}>
                  <div className="mb-4 opacity-50 transition-all duration-300 hover:opacity-100">
                    {/* Flex container for image and text */}
                    <div className="flex items-center justify-center gap-4 w-full">
                      {/* Image */}
                      <div className="relative w-10 h-10 xl:w-20 xl:h-20"> {/* Image size change on mobile */}
                        <ImageClient
                          src={item.image}
                          alt=""
                          height={100}
                      ...
                      <div className="flex flex-col justify-center">
                        <h3 className="">{item.action}</h3> {/* Adjust text size for mobile */}
                      </div>
                    </div>
                    <div className="border-t-2 border-white py-4 mt-4 sm:border-t-1 sm:py-2 md:border-t-2 md:py-4 transition-all duration-300 group-hover:border-green-800">
                      <p className="">{item.description}</p> {/* Adjust text size for mobile */}
                    </div>
                  </div>
                </div>
              )
